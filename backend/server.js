...
app.delete('/api/admin/messages/:messageId', (req, res) => {
  ...
  try {
    ...
    // В общем чате (chatId = 'general') могут удалять только администраторы
    // В других чатах могут удалять все пользователи
    if (chatId === 'general' && userData.is_admin !== 1) {
      return res.status(403).json({ error: 'В общем чате сообщения могут удалять только администраторы' });
    }
...
const botKnowledge = {
  ...
  responses: {
    ...
    'морожен': '🐻 Медведи не едят мороженое, но я слышал, что компания УРСА производит самое вкусное мороженое! 🍦',
    'урса': '🏢 Компания УРСА — это не только строительные материалы, но и, по слухам, самое вкусное мороженое! 🍦🐻'
  },
...
📝 *Важно: заполните свой профиль!*
Пожалуйста, зайдите в свой профиль и укажите:
• 📋 Должность
• 📞 Рабочий номер телефона
• 📱 Мобильный телефон

Это поможет коллегам быстро с вами связаться!
...
/помощь — все команды`;
        ...
        const welcomeButtons = [
          { label: '👤 Заполнить профиль', action: '/профиль' },
